...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/24 12:14:31 by gfaviere          #+#    #+#             */
/*   Updated: 2021/02/23 11:05:16 by gfaviere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				finish_init(t_game *game)
{
	if (!init_window(&game->window, &game->config))
	{
		return (exit_error(game,
			"Error: mlx failed to create window or image.\n"));
	}
	find_start_pos(&game->config, &game->camera);
	find_start_angle(&game->config, &game->camera);
	if (!load_textures(game))
		return (exit_error(game, "Error: failed to load texture(s).\n"));
	if (!find_sprites(game))
		return (exit_error(game, "Error: failed to malloc sprites.\n"));
	count_items(game);
	make_tables(game);
	return (1);
}
...
int				screenshot(t_game *game)
{
	...
	update_window(game);
	if (!save_bmp(game))
		exit_error(game, "Error: failed to save screenshot.");
